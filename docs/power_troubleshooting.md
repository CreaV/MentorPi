...
**隔离性：已确认为"差"（2026-07-05 实测）**。STM32 开关关掉时，数据线 VBUS 会把
板子寄生点亮：IMU 照常发数、串口正常、`/battery` 读到 **4.44V**（寄生 5V 轨残压）
——但电机驱动完全没电，车轮不转。结论：

- STM32 板会从 Pi 的 USB 口偷电（可达几百 mA），**挤占 Pi 的 USB 过流预算**，
  加剧下述过流问题
- **`/battery` ≈ 4.4V 是"电源开关没开"的特征签名**（正常 2S 电压 ≈ 6.6–8.4V）。
  排查"电机不转/车不动"时先看这个值
- IMU/串口活着 ≠ 电机有电，两者供电路径不同，别被骗
